<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>填空题管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图区 -->
    <el-card>
      <!-- 搜索区域/添加区域 -->
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input
            ...
            @clear="getFillGapQuesList"
          >
            <el-button
              slot="append"
              icon="el-icon-search"
              @click="getFillGapQuesList"
            ></el-button>
          </el-input>
        </el-col>
        <el-col :span="4">
      ...
      <!-- 数据显示区 -->
      <el-table height="650px" border stripe :data="fillGapQuesList">
        <!-- 所有数据列 -->
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="addFillGap">确 定</el-button>
        </span>
      </el-dialog>
      <!-- 修改对话框 -->
      <el-dialog
      ...
      >
        ...
        <span slot="footer">
          ...
          <el-button type="primary" @click="updateFillGap">确 定</el-button>
        </span>
      </el-dialog>
...
export default {
  ...
  created() {
    this.getFillGapQuesList();
  },
  methods: {
    async getFillGapQuesList() {
      const { data: res } = await this.$http.get("showFillGapQuesLists", {
        params: this.queryInfo,
      });
      if (res.code !== 200) {
        return this.$message.error("获取用户列表失败! ");
      }
      this.fillGapQuesList = res.data.fillGapQuesList;
    ...
    addFillGap() {
      ...
      this.$refs.addFormRef.validate(async (isValid) => {
        ...
        const { data: res } = await this.$http.post("addFillGap", this.addForm);
        if (res.code != 200) {
    ...
    async deleteById() {
      const { data: res } = await this.$http.delete(
        "deleteFillGapByQid/" + this.deleteQid
      );
      if (res.code != 200) {
